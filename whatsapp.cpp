...
struct NodoConversacion {
	infoConver info;
	NodoConversacion* sig;
};

int buscar(Contactos vec[], int len, int idUsuarioEmisor);
NodoConversacion* buscar(NodoConversacion *p, int idConversacion);
string VistaPrevia(Mensaje mensaje);
string NumeroTelefonicoUsuario(int idUsuario);
void EmitirNotificacion(string titulo, string descripcion);
void agregarNodo(NodoConversacion* &p, infoConver x);
void notificarMensajeRecibido (NodoConversacion* conversaciones, Contactos vec[], 
int len, int idConversacion, int idUsuarioEmisor, Mensaje mensaje);

struct estacion {
	string nombre;
	int fechaInicio;  //mesDia MMDD
	int fechaFin;  //mesDia MMDD
};
...
void agregarNodo(NodoReserva* &p, infoReser x);

int main(int argc, char** argv) {
	/* Carga de archivo reservas
	FILE * arch = fopen("reservas.dat","wb+"); 
	reserva reg;
	
	reg.zona = 'A';
	reg.mesDia = 111;
	reg.anio = 2021;
	reg.cantReservas = 10000;
	
	fwrite(&reg,sizeof(reserva),1,arch);
	
	reg.zona = 'A';
	reg.mesDia = 206;
	reg.anio = 2021;
	reg.cantReservas = 7500;
	...
	fread(&reg,sizeof(reserva),1,arch);
	
	while(!feof(arch)){
		cout << reg.zona << " " << reg.mesDia << " " << reg.anio << " " << reg.cantReservas << endl;
 	...
 	*/
 	...
 	//Desarrollo del final WhatsApp
 	
 	/*NodoConversacion* conversaciones = NULL;
 	Contactos vec[200] = {{1,"Pepito",false}};
 	int len = 1;
 	
 	infoConver info;
 	info.idConversacion = 3542;
 	info.nombre = "Club";
 	info.notificar = true;
 	info.vistaPrevia = false;

 	agregarNodo(conversaciones, info);
 	
 	
 	info.idConversacion = 8349;
 	info.nombre = "";
 	info.notificar = true;
 	info.vistaPrevia = false;
 	...
 	notificarMensajeRecibido(conversaciones, vec, len, 8349, 2, m);
 	*/
 	
 	//Final secretar�a de turismo
 	
 	FILE* arch = fopen("reservas.dat","rb");
 	reserva reg;
 	
 	NodoReserva* listaCosta  = NULL;
 	NodoReserva* listaMontania = NULL;
 	int dia;
 	int mes;
 	estacion est;
 	infoReser info;
 	...
 	while(!feof(arch)){
 		dia = reg.mesDia % 100;
 		mes = reg.mesDia / 100;
 		est = estacionDelAnio(dia, mes);
 		
		if(reg.zona == 'A'){ // temporada alta -> Verano
 		 	if(est.nombre == "Verano"){
 		 		info.anio = reg.anio;
 		 		info.dia = dia;
 		 		info.mes = mes;
 		 		info.cantReservas = reg.cantReservas;
				agregarNodo(listaCosta, info);	
			}
		} 
		  
		 if(reg.zona == 'M'){ // temporada baja -> Verano
		 	if(est.nombre == "Verano"){
 		 		info.anio = reg.anio;
 		 		info.dia = dia;
 		 		info.mes = mes;
 		 		info.cantReservas = reg.cantReservas;
				agregarNodo(listaMontania, info);	
			}
		 }
	...
	cout << "Costa: Temporada Alta (Verano)" << endl;
	
	NodoReserva* aux = listaCosta;
	
	while(aux != NULL){
		cout<< "Anio: " << aux->info.anio <<  "  Mes: " << aux->info.mes << "  Dia: " << aux->info.dia 
		<< "  Cantidad de Reservas: " << aux->info.cantReservas << endl;                  
		
		aux = aux->sig;
	}
	
	cout << endl;
	cout << "Montania: Temporada Baja (Verano)" << endl;
...
int buscar(Contactos vec[], int len, int idUsuarioEmisor){
	...
	while(i < len && vec[i].idUsuario != idUsuarioEmisor) {
		i++;
	}
...
string NumeroTelefonicoUsuario(int idUsuario){
	//no tenemos el nro de cel del usuario
	return "+549111111111";
}
...
void agregarNodo(NodoConversacion* &p, infoConver x){
	...
	NodoConversacion* nuevo = new NodoConversacion();
	nuevo->info = x;
	nuevo->sig = NULL;
...
int len, int idConversacion, int idUsuarioEmisor, Mensaje mensaje) {
	...
	NodoConversacion* converBuscada = buscar(conversaciones, idConversacion);
	int posVector = buscar(vec, len, idUsuarioEmisor);
	
	string nombreContacto;
	
	if(posVector != -1){ // esta en la lista de contactos
		nombreContacto = vec[posVector].nombreContacto;
	} else { // no esta en la lista de contactos
		nombreContacto = NumeroTelefonicoUsuario(idUsuarioEmisor);
	}
	
	if(converBuscada->info.notificar && (!vec[posVector].bloqueado || posVector == -1)){
		
		if(converBuscada->info.nombre != "" ){  // es grupal
			titulo = converBuscada->info.nombre;
			
			if(converBuscada->info.vistaPrevia){  //es grupal con vista previa
				descripcion = nombreContacto + ": " + VistaPrevia(mensaje);
			} else { // es grupal sin vista previa
			...
			if(converBuscada->info.vistaPrevia){ // directa con vista previa
		...
		cout << "Notificaciones apagadas o usuario bloqueado" << endl;
	}
