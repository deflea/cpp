...
int main(int argc, char** argv) {
	/* Cargar archivo para Final elecciones vecinales
	FILE * arch = fopen("eleccionVecinal.dat","wb+");
	eleccion reg;
	
	strcpy(reg.localidad,"Pinamar");
	reg.lista = 1;
	reg.votos = 3000;
	
	fwrite(&reg,sizeof(eleccion),1,arch);
	
	strcpy(reg.localidad,"Carilo");
	reg.lista = 1;
	reg.votos = 750;
	
	fwrite(&reg,sizeof(eleccion),1,arch);
	
	strcpy(reg.localidad,"Ostende");
	reg.lista = 1;
	reg.votos = 500;
	
	fwrite(&reg,sizeof(eleccion),1,arch);
	
	strcpy(reg.localidad,"Valeria del Mar");
	reg.lista = 1;
	reg.votos = 250;
	...
	while(!feof(arch)){
		cout << reg.localidad << " " << reg.lista << " " << reg.votos << endl;
		fread(&reg,sizeof(eleccion),1,arch);	
	}
	fclose(arch);
	*/
	...
	localidad localides[4] = {{"Pinamar"},{"Carilo"},{"Ostende"},{"Valeria del Mar"}};
	
	int lenLoc = 4;
	
	FILE * arch = fopen("eleccionVecinal.dat","rb");
	
	
	calcularListaGanadora(matLocLista, localides, lenLoc, arch);
	
	
	
	/* Final con vectores
	candidato docentes[15] = {
	...
	resultado vecResul[5];
	int lenResul = 0;
	
	GenerarListaConsejerosDocentes(docentes, lenDocentes, vecResul, lenResul);
	
	cout << "Los 5 candidatos ganadores son: " << endl;
	
	for(int i = 0; i < 5; i++){
		cout << vecResul[i].pos << " " << vecResul[i].cand.lista << " " << vecResul[i].cand.nombreCandidato << endl;
	}
	*/
